...
//   const canManageProducts = user?.roleName  === 'Admin' || user?.role === 'Manager';
...
const ProductsPage = () => {
  ...
  const canManageProducts = user?.roleName === 'Admin' || user?.roleName === 'Manager';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error.response?.status === 409) {
        setError(error.response.data.message || 'Sản phẩm đã tồn tại');
...
const EditProductModal = ({ product, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await productApi.update(product.id, requestData);
      onSuccess();
    } catch (error) {
